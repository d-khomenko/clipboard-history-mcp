...
mod macos_pb {
    ...
    pub fn write_image_macos(bytes: &[u8], mime: &str) -> Result<()> {
        ...
        autoreleasepool(|_| {
            ...
            let written = pb.setData_forType(Some(&data), uti);
            if !written {
    ...
    pub fn write_files_macos(paths: &[PathBuf]) -> Result<()> {
        let file_uti = unsafe { NSPasteboardTypeFileURL };
        autoreleasepool(|_| {
            let pb = NSPasteboard::generalPasteboard();
            pb.clearContents();
            for path in paths {
                ...
                let written = pb.setString_forType(&ns_str, file_uti);
                if !written {
                    return Err(anyhow::anyhow!(
                        "NSPasteboard.setString_forType returned false"
                    ));
                }
            }
            Ok(())
